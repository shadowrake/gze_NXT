...
import {getTeamsSoon} from '../lib/prisma/read/teamsSoon.js'
  ...
  export default async function team_main() {
    ...
    const apply = teamsSoon.filter((teams) => teams.keyword === "Soon")
          ...
          <div className="mt-6 grid grid-cols-1 gap-x-8 gap-y-8 sm:grid-cols-2 sm:gap-y-10 lg:grid-cols-3">
            {apply.map((teams) => (
              ...
              <div  className="group relative">
                ...
                <div className="bg-red-100 relative group">
                  <img src={teams.imageSrc} alt={teams.imageAlt} className="object-contain h-190 w-290 object-center" />
                  <div className="opacity-0 group-hover:opacity-100 duration-300 absolute inset-x-0 inset-y-0 flex justify-center items-center text-xl bg-green-200 md:bg-opacity-80 text-black font-semibold">Apply now</div>
                </div>
                </div>
              </div>
              </a>
